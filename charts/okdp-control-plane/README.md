...
To install the chart: a Kubernetes cluster, Helm >= 3.8, and the kubauth
`OidcClient` CRD unless `oidcClient.enabled=false`.
...
- [kubauth](https://github.com/kubotal/kubauth) as OIDC provider, at the URL set
  in `okdp-control-plane-ui.oidc.authority`, allowing the console origin in CORS;
...
provides everything above on a local Kind cluster.
